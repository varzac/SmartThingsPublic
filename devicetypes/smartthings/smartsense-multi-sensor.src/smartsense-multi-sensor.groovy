...
metadata {
	definition (name: "SmartSense Multi Sensor (binding table)", namespace: "smartthings", author: "SmartThings", category: "C2") {
...
def parse(String description) {
	...
	if (description?.startsWith('catchall:')) {
		result = parseCatchAllMessage(description).collect { createEvent(it) }
	} else if (description?.startsWith('temperature: ')) {
		result = parseCustomMessage(description)
		result = result ? createEvent(result) : null
