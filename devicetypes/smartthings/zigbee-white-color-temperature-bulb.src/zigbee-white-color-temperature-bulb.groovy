...
def parseBindingTableResponse(description) {
    ...
    if (descMap["clusterInt"] == 0x8033) {
        ...
        // Work around for ember firmware bug
        def numEntriesPos = 4
        def direction = descMap["direction"]
        if ((!(direction in ["00", "01"])) || (data.length() < numEntriesPos) || (Integer.parseInt(data[numEntriesPos], 16) > 3)) {
            data = [direction] + data
        }
